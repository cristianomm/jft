...
public class ClearBMFInvoiceExtractor extends ClearInvoiceExtractor {
    ...
    private static String vals_0 = date + "[\\s]+" + invoice;
	...
	String v0 = extractValue(text, vals_0, 1);
	inv.date = extractValue(v0, date, 0);
	inv.invoice = extractValue(v0, invoice, 0);
	...
	if(ret.length()>0){
	    ret = ret.replaceAll("\n", "").replaceAll("\r", "");
	    ret = ret.trim().replace("C", "").replace("R$", "").replace(" ", "").replace("|", "");
	    if(ret.contains("D")){
		ret = "-" + ret.replace("D", "").replaceFirst("-", "");
	    }	
	}
	return ret;
    }
	...
	try {
	    ...
	    Logging.getInstance().log(getClass(), "Error extracting values: " + e.getMessage(), Level.ERROR);
	}

	return invs;
    }

    private static ArrayList<File> listInvoices(File dir) {
	ArrayList<File> files = new ArrayList<>();
	for(File f : dir.listFiles()) {
	    if(f.isFile()) {
		files.add(f);
	    }else if(!f.getName().contains("Bovespa")) {
		files.addAll(listInvoices(f));
	    }
	}
	return files;
    }
	 ...
	 */
	//\\2014\\BMF\\
	String path = "D:\\Disco\\Users\\Cristiano\\Documents\\Bolsa\\Notas Corretagem\\2017\\BMF";

	FileOutputStream fos = new FileOutputStream(new File("BMF_2017.csv"));
	...
	List<File> files = listInvoices(new File(path));
	for(File f:files) {
	    System.out.println("Processing: " + f.getName());

	    Properties p = new Properties();
	    p.put("filename", f.getPath());
