...
public class NewsStream extends Stream {
	...
	public void createSessionSettings() {
		try {
			sessionSettings = new SessionSettings(
					Thread.currentThread().getContextClassLoader().getResourceAsStream("NewsService.cfg"));
		} catch (ConfigError e) {
	...
	private void loadNews() {
		try {
			...
			CSV csv = new CSV(Thread.currentThread().getContextClassLoader().getResource("News.csv").getPath(), ";",
					"#");
			while (csv.hasNext()) {
