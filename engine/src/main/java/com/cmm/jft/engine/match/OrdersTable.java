...
public class OrdersTable {
	...
	this.stopQueue = Collections.synchronizedSortedMap(
		new TreeMap<Double, SortedMap<LocalDateTime, Orders>>());
    }
	...
	if(order != null && order.getOrderID() >0 && order.getClOrdID() != null) {
		...
		orders.put(order.getPrice(), 
			Collections.synchronizedSortedMap(new TreeMap<LocalDateTime, Orders>()));
	    }
	...
	if(ordr.getOrderType() == OrderTypes.Stop || ordr.getOrderType() == OrderTypes.StopLimit){
		...
		stopQueue.put(ordr.getStopPrice(), (TreeMap<LocalDateTime, Orders>) Collections.synchronizedMap(new TreeMap<LocalDateTime, Orders>()));
	    }
