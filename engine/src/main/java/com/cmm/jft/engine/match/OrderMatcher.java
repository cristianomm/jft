...
public class OrderMatcher implements MessageSender {
	...
	public OrderMatcher(double protectionLevel, MarketDataStream umdf, OrdersTable buyTable, OrdersTable sellTable) {
		this.umdf = umdf;
	...
	public void cancelOrder(Orders ordr, CancelTypes cancelType) throws OrderException {
		...
		// UpdateActions.Delete:UpdateActions.Change;
	...
	/**
	 * For bids, the protection price calculated is by adding an offset to the last
	 * trade price. For offers, the offset is subtracted from the last trade. The
	 * protection price cannot be specified in the incoming order.
	 * 
	 * @param ordr
	 */
	...
	private void restateOrder(Orders order) {
		try {
			if (order.getMaxFloor() > 0 && order.getOrderStatus() == OrderStatus.PARTIALLY_FILLED
	...
	private void matchDay(Orders aggrOrder, Orders bookOrder, double fillPrice) throws OrderException {
		...
		if (aggrOrder.getMinVolume() > 0) {
			...
			qtyToFill = bookOrder.getLeavesVolume() < aggrOrder.getLeavesVolume() ? bookOrder.getLeavesVolume()
	...
	private void matchFOK(Orders aggrOrder, Orders bookOrder, double fillPrice) throws OrderException {
		...
		if (aggrOrder.getVolume() <= bookOrder.getLeavesVolume()) {
			if (aggrOrder.getMinVolume() > 0) {
				...
				/*
				 * nao ha qty minima, executa com a qty da ordem agressora pois a ordem que esta
				 * no book possui qty suficiente para executar a ordem
				 */
	 ...
	 * @return a quantidade possivel de ser executada conforme a quantidade
	 *         existente no book ou zero caso nao possua quantidade minima
	 *         suficiente para a execucao da ordem agressora.
	...
	private boolean matchLimit(Orders order) throws OrderException {
		...
		while (bookOrder != null
				&& (order.getOrderStatus() == OrderStatus.NEW || order.getOrderStatus() == OrderStatus.PARTIALLY_FILLED
	...
	private boolean matchMarketWithProtection(Orders aggrOrder) throws OrderException {
		...
		while ((aggrOrder.getOrderStatus() == OrderStatus.NEW
	...
	private boolean matchMarketToLimit(Orders aggrOrder) throws OrderException {
		...
		/*
		 * apos a primeira execucao, ajusta a ordem para limit e ajusta o preco da ordem
		 * para o preco do ultimo negocio executado
		 */
		boolean exec = false;
	...
	// if (aggrOrdr.getOrderStatus() == OrderStatus.PARTIALLY_FILLED ||
	// aggrOrdr.getOrderStatus() == OrderStatus.NEW) {
	//
