...
public class OrderMatcher implements MessageSender {
	...
	private TreeMap<Double, TreeMap<Date, Orders>> queue;

	public StopOrderReleaser(TreeMap<Double, TreeMap<Date, Orders>> queue) {
	    this.queue =  queue;
	}
	...
	public void run() {
		...
		if(queue.containsKey(lastPrice) && !queue.get(lastPrice).isEmpty()){
		    TreeMap<Date, Orders> stops = queue.get(lastPrice);
		    while(!stops.isEmpty()){
			try {
    ...
    public OrderMatcher(MatchTypes matchTypes, double protectionLevel, 
	    MarketDataChannel umdf, BookTable buyTable, BookTable sellTable) {
	this.umdf = umdf;
