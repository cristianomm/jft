...
public class MarketDataStream extends Stream {
	...
	public void informOpenPrice(String symbol, double openPrice) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.openPriceEntryInc(UpdateActions.New,
					SecurityService.getInstance().provideSecurity(symbol), openPrice, getRptSeq(symbol));
	...
	public void informClosePrice(String symbol, double closePrice) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.closePriceEntryInc(
					SecurityService.getInstance().provideSecurity(symbol), closePrice, getRptSeq(symbol));
	...
	public void informHighPrice(String symbol, double highPrice) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.highPriceEntryInc(UpdateActions.New,
					SecurityService.getInstance().provideSecurity(symbol), highPrice, getRptSeq(symbol));
	...
	public void informLowPrice(String symbol, double lowPrice) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.lowPriceEntryInc(UpdateActions.New,
					SecurityService.getInstance().provideSecurity(symbol), lowPrice, getRptSeq(symbol));
	...
	public void informVWAPPrice(String symbol, double vwapPrice) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.vwapPriceEntryInc(UpdateActions.New,
					SecurityService.getInstance().provideSecurity(symbol), vwapPrice, getRptSeq(symbol));
	...
	public void informTradeVolume(String symbol, int numOfTrades, double financialVolume, double tradedVolume) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder.tradeVolumeEntryInc(
					SecurityService.getInstance().provideSecurity(symbol), numOfTrades, financialVolume, tradedVolume,
					getRptSeq(symbol));
	...
	public void informEmptyBook(String symbol) {
		try {
			MarketDataIncrementalRefresh.NoMDEntries entry = encoder
					.emptyBookEntryInc(SecurityService.getInstance().provideSecurity(symbol), getRptSeq(symbol));
	...
	/**
	 * Mercado por Oferta, Esse tipo contem informacoes sobre cada uma das ofertas,
	 * com cada registro representando uma unica ordem. A BM&FBOVESPA sempre enviara
	 * a profundidade total para livro agregado por oferta, i.e. o cliente recebera
	 * atualizacoes para todas as ofertas contidas no livro, mesmo que seja a ultima
	 * (pior pre�o). Para livros mais �rasos� isso pode nao ser um problema, mas
	 * para livros mais profundos atualizacoes completas nem sempre sao relevantes,
	 * alem de consumir banda.
	 */
	public void mbo() {
	...
	/**
	 * Mercado por Preco, O livro agregado por pre�o contem as informacoes
	 * individuais de pre�o, em que cada registro representa a agrega��o de todas as
	 * ofertas (e suas quantidades) por aquele pre�o. Alem de quantidade e pre�o, o
	 * livro agregado por pre�o informa o numero de ofertas inseridas por
	 * determinado pre�o. A BM&FBOVESPA preestabelece a profundidade do livro que
	 * estara disponivel por instrumento, com atribuicao automatica de 5. Isso
	 * significa que o cliente somente recebera os 5 melhores niveis de pre�o no
	 * livro.
	 */
	public void mbp() {
	...
	/**
	 * Topo do Book. Apresenta as melhores ofertas de compra e venda agrupadas por
	 * preco em somente um nivel
	 * 
	 */
	public void tob() {
