...
public class OrderService {
	 ...
	 */
	private Orders createOrder(Security security, Side side, double price, int volume, OrderTypes orderType,
			TradeTypes tradeType) throws OrderException {
	...
	public List<Orders> newOrder(OrderTypes orderType, Security security, Side side, int volume, double price,
			double stopLoss, double stopGain) {
		...
		case Limit:
			orders = newLimitOrder(security, side, volume, price);
			if (orders != null) {
				ordrs.add(orders);
				createLA = true;
			}
		...
		case Market:
			orders = newMarketOrder(security, side, volume);
			if (orders != null) {
				ordrs.add(orders);
				createLA = true;
			}
	...
	private Orders newLimitOrder(Security security, Side side, int volume, double limitPrice) {
		Orders ordr = null;
		try {
	...
	private Orders newStopOrder(Security security, Side side, int volume, double stopPrice) {
		Orders ordr = null;
		try {
	...
	private Orders newMarketOrder(Security security, Side side, int volume) {
		Orders ordrs = null;
		try {
	...
	private Orders[] createLossAndGain(Security security, Side sideExit, int volume, double price, double stopLoss,
			double stopGain) {
	...
	public Orders createAndPersistOrder(OrderTypes orderType, Side side, String symbol, int volume, double price,
			double stopPrice) throws InvalidOrderException, DataBaseException {
		...
		try {
			...
			List<Security> l = (List<Security>) DBFacade.getInstance().queryNamed("Security.findBySymbol", params);
			if (l == null || l.size() == 0) {
			...
			Logging.getInstance().log(OrderService.class, "Erro ao retornar Ordem.", e, Level.ERROR, false);
	...
	// }
	//
	// private void addEvent(Event event) throws OrderException,
	// DataBaseException {
	//
	...
	//
	// order = (Orders) DBFacade.getInstance().attachToSession(order,
	// order.getOrderID());
	//
	// // adiciona a execucao
	...
	// ExchangeTradingService.getInstance().registerTrade(order.getTradeID());
	// }
	//
