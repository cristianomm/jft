...
public class JournalService {
	 ...
	 * parametro.
	 * 
	 * @param object Enumeracao que informa o tipo do objeto relacionado a regra de
	 *               distribuicao
	 * @return regra de distribuicao relacionada ao objeto.
	 * @throws RegistrationException Caso nao exista uma regra de distribuicao
	 *                               relacionada.
	 */
	public DistributionRule getDistributionRule(Objects object) throws RegistrationException {
		...
		try {
			lres = (List<DistributionRule>) DBFacade.getInstance().queryNamed("DistributionRule.findByObjectRule",
					params);
	...
	public JournalEntry registerEntry(JournalEntry journalEntry, DistributionRule distrRule)
	...
	public void registerEntry(JournalEntry journalEntry, Account creditAccount, Account debitAccount, double value,
			String descr) throws RegistrationException, DataBaseException {

		// verificar se o proprio je esta aberto
		if (journalEntry.getJournalStatus() != JournalStatus.OPEN) {
			throw new RegistrationException("Invalid JournalEntry State: " + journalEntry.getJournalStatus());
		}
		...
		EntryRegister creditReg = new EntryRegister(EntryType.CREDIT, crv, dbtv, descr, defCurr, creditAccount,
				debitAccount);
	...
	public JournalEntry closeEntry(JournalEntry je) {
		...
		try {
			...
			Logging.getInstance().log(getClass(), "Erro ao fechar registro", e, Level.ERROR, false);
		}
		return je;
	}
	...
	public JournalEntry cancelEntry(JournalEntry je) {
		...
		try {
			...
			Logging.getInstance().log(getClass(), "Erro ao cancelar registro", e, Level.ERROR, false);
		}
		return je;
	}

	public void addCreditRegister(JournalEntry journalEntry, Account creditAccount, double value, String description)
			throws RegistrationException {
