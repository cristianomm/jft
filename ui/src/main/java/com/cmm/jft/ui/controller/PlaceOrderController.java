...
public class PlaceOrderController extends AbstractController {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		txtSecurity.textProperty().addListener(
				(obsrv, oldValue, newValue) -> {
					if(newValue != null && !newValue.equalsIgnoreCase(oldValue)){
						try{
							securityVO = Memory.getInstance().getSecurityVO();
							changeSpinners(securityVO.getSymbol());
						}catch(Exception e){
		...
		cmbOrderTypes.valueProperty().addListener(
		...
		/*
		cmbValidityTypes.valueProperty().addListener(
				(observable, oldValue, newValue) -> {
					if(newValue != oldValue){
						dtDate.setValue(LocalDate.now());
					}
		...
		btnBuy.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				try{
					...
					String symbol = securityVO.getSymbol();
	...
	private void changeSpinners(String symbol){
		...
		spnVolume.setValueFactory(new SpinnerValueFactory.DoubleSpinnerValueFactory(minVolume, maxVolume, minVolume, volumeIncr));
